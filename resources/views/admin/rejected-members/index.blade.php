...
@section('title', 'Rejected Members')
@section('page-title', 'Rejected Members')
@section('page-subtitle', 'Review rejected registrations and re-accept when appropriate')

@section('page-actions')
<a href="{{ route('admin.waiting-approvals.index') }}" class="btn-outline">Waiting Approvals</a>
...
<div class="card">
    <form id="rejected-filter-form" method="GET" action="{{ route('admin.rejected-members.index') }}" class="border-b border-slate-100 p-4" data-auto-filter>
    ...
    @if (auth()->user()->canManageMembers() && $members->count())
        <form id="rejected-bulk-form" method="POST" action="{{ route('admin.rejected-members.bulk') }}" class="border-b border-slate-100 bg-surface/70 px-4 py-3" data-member-bulk-form>
            @csrf
            <div class="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
                ...
                <div class="flex flex-col gap-2 sm:flex-row sm:flex-wrap sm:items-center">
                    <select name="action" required class="bulk-action form-select sm:w-56" disabled>
                        ...
                        <option value="reaccept">Re-accept</option>
                    </select>
        ...
        <table class="data-table">
            <thead>
                <tr>
                    @if (auth()->user()->canManageMembers() && $members->count())
                        <th class="w-10">
                            <input type="checkbox" class="bulk-select-all h-4 w-4 rounded border-slate-300 text-brand-green focus:ring-brand-green" title="Select all on this page" form="rejected-bulk-form">
                        </th>
                    @endif
                    <th>Member</th>
                    <th>Unique ID</th>
            ...
            <tbody>
                @forelse ($members as $member)
                    <tr>
                        @if (auth()->user()->canManageMembers())
                            <td>
                                <input type="checkbox" name="member_ids[]" value="{{ $member->id }}" class="member-bulk-checkbox h-4 w-4 rounded border-slate-300 text-brand-green focus:ring-brand-green" form="rejected-bulk-form">
                            </td>
                        @endif
                        <td>
                        ...
                        <td class="max-w-xs text-xs text-muted">{{ $member->rejection_reason ?: '—' }}</td>
                        <td class="text-xs text-muted">
                            {{ \App\Support\SriLankaDate::datetime($member->updated_at) }}
                            @if ($member->approver)
                                <span class="mt-1 block">by {{ $member->approver->name }}</span>
                            @endif
                        </td>
                        <td>
                            <div class="flex flex-wrap items-center justify-end gap-2">
                                ...
                                @if (auth()->user()->canManageMembers())
                                    <form method="POST" action="{{ route('admin.members.re-accept', $member) }}" data-confirm="Re-accept {{ $member->displayName() }}? They will become an active approved member.">
                                        @csrf
                                        <button type="submit" class="btn-primary px-3 py-1.5">Re-accept</button>
                                    </form>
                                @endif
                            </div>
                        </td>
                ...
                @empty
                    <tr>
                        <td colspan="{{ auth()->user()->canManageMembers() ? 7 : 6 }}" class="py-14 text-center text-muted">
                            No rejected members.
                        </td>
                    </tr>
                @endforelse
