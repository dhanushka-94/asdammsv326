...
<div class="relative min-h-screen overflow-hidden bg-surface">
    ...
    <div class="relative mx-auto flex min-h-screen max-w-lg flex-col justify-center px-4 py-10 sm:px-6">
        ...
        <div class="card overflow-hidden">
            <div class="bg-brand-green px-5 py-5 text-white">
                <h1 class="font-display text-2xl font-bold">Set a new password</h1>
                <p class="mt-1 text-sm text-white/85">For security, change your default NIC password before continuing.</p>
            </div>

            <form method="POST" action="{{ route('member.password.update') }}" class="space-y-5 p-5 sm:p-8">
                @csrf
                @method('PUT')

                <div class="rounded-xl bg-surface p-4">
                    <p class="text-xs font-semibold uppercase tracking-wide text-muted">Signed in as</p>
                    <p class="mt-1 font-semibold text-ink">{{ $member->displayName() }}</p>
                    <p class="text-sm text-muted">{{ $member->nic }} · {{ $member->unique_id }}</p>
                </div>
                ...
                <div>
                    ...
                    <p class="mt-1 text-xs text-muted">At least 8 characters. Do not use your NIC.</p>
                </div>
        ...
        <form method="POST" action="{{ route('member.logout') }}" class="mt-4 text-center">
            @csrf
